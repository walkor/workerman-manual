...
When you want to know what a process is doing, you can track all system calls of a process using the `strace` command.

1. Running `php start.php status` will display information about the Workerman-related processes as follows: 
...
2. For example, if we want to know what the gateway process with PID 10354 is doing, we can run the command `strace -p 10354` (may require root privileges) as shown below:
...
3. Each line represents a system call, and from this information, we can easily see what the process is doing. It allows us to locate where the process is stuck, whether it's in connection or reading network data, etc.

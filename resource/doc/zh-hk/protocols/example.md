...
struct
{
  ...
  char         file[total_len - BinaryTransfer::PACKAGE_HEAD_LEN - name_len]; // 文件數據
}
```
### 協議樣本
...
其中首部四字節\*號代表一個網絡字節序的unsigned int數據，為不可見字符，第5個\*是用一個字節存儲文件名長度，緊接著是文件名，接著是原始的二進制文件數據
...
class BinaryTransfer
{
    ...
    public static function encode($data)
    {
        // 可以根據自己的需要編碼發送給用戶端的數據，這裡只是當做文本原樣返回
        return $data;
    }
}
```
...
### 客戶端文件 client.php （這裡用php模擬客戶端上傳）
```php
<?php
/** 上傳文件客戶端 **/
// 上傳地址
...
json+換行，json中包含了檔案名以及base64_encode編碼（會增大1/3的體積）的檔案資料
...
class TextTransfer
{
    ...
    public static function decode($recv_buffer)
    {
        ...
        // 取出base64_encode後的檔案資料
        $file_data = $package_data['file_data'];
        // base64_decode還原回原來的二進制檔案資料
    ...
    public static function encode($data)
    {
        // 可以根據自己的需要編碼發送給客戶端的資料，這裡只是當做文本原樣返回
        return $data;
    }
}
...
### 客户端檔案 textclient.php （這裡用php模擬客戶端上傳）
```php
<?php
/** 上傳檔案客戶端 **/
// 上傳地址

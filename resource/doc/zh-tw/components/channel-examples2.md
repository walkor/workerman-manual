...
``` php
    ...
    $worker->onMessage = function(TcpConnection $con, $data){
        // 加入群組訊息{"cmd":"add_group", "group_id":"123"}
        // 或者 群發訊息{"cmd":"send_to_group", "group_id":"123", "message":"這個是訊息"}
        ...
        switch($cmd) {
            ...
            case "add_group":
                ...
                // 記錄這個連線加入了哪些群組，方便在onclose的時候清理group_con_map對應群組的資料
                $con->group_id = isset($con->group_id) ? $con->group_id : array();
                $con->group_id[$group_id] = $group_id;
                break;
            // 群發訊息給群組
            case "send_to_group":
                // Channel\Client給所有伺服器的所有進程廣播分組發送訊息事件
    ...
    // 這裡很重要，連線關閉時把連線從全域群組資料中刪除，避免內存洩漏
    $worker->onClose = function(TcpConnection $con){
        global $group_con_map;
        // 遍歷連線加入的所有群組，從group_con_map刪除對應的資料
...
打開chrome瀏覽器，按F12打開調試控制台，在Console一欄輸入(或者把下面程式碼放入到html頁面用js運行)
...
ws.onopen = function() {
    ...
    ws.send('{"cmd":"send_to_group", "group_id":"123", "message":"這個是訊息"}');
};
